...
abstract class ITime {
  ...
  static const int maxDays =
      maxMillis ~/ TimeConstants.millisecondsPerDay; // (1 << 24) - 1;
  // ~maxDays would be 4190717304 on JS (-104249992 is the correct number)
  static const int minDays =
      -104249992; // ~maxDays; <-- doesn't work in JS // todo: may hard encode if this makes unit tests not work

  // todo: Convert to BigInt for Dart 2.0
  static final BigInt minNanoseconds =
      BigInt.from(minDays) * BigInt.from(TimeConstants.nanosecondsPerDay);
  static final BigInt maxNanoseconds = (BigInt.from(maxDays) + BigInt.one) *
          BigInt.from(TimeConstants.nanosecondsPerDay) -
      BigInt.one;
  ...
  static const int minMillis = Platform
      .intMinValueJS; // -9007199254740993; // Utility.intMinValueJS; // was -maxMillis; very shortly was ~maxMillis (which I guess doesn't work well in JS)

  static Time plusSmallNanoseconds(Time span, int nanoseconds) =>
      span._plusSmallNanoseconds(nanoseconds);
  ...
  static Time trusted(int milliseconds, [int nanosecondsInterval = 0]) =>
      MillisecondTime(milliseconds, nanosecondsInterval);
  static Time untrusted(int milliseconds, [int nanoseconds = 0]) =>
      Time._untrusted(milliseconds, nanoseconds);
  ...
  static int nanosecondOfDurationDay(Time time) =>
      time._nanosecondOfDurationDay;
...
abstract class Time implements Comparable<Time> {
  ...
  static const Time oneMicrosecond =
      const MillisecondTime(0, TimeConstants.nanosecondsPerMicrosecond);
  static const Time oneMillisecond = const MillisecondTime(1, 0);
  static const Time oneSecond =
      const MillisecondTime(TimeConstants.millisecondsPerSecond, 0);
  static const Time oneDay =
      const MillisecondTime(TimeConstants.millisecondsPerDay, 0);
  static const Time oneWeek =
      const MillisecondTime(TimeConstants.millisecondsPerWeek, 0);
  ...
  static final Time maxValue = Time(
      days: ITime.maxDays, nanoseconds: TimeConstants.nanosecondsPerDay - 1);
  ...
  factory Time._untrusted(int milliseconds, [int nanoseconds = 0]) {
    ...
    if (nanoseconds < _minNano) {
      var delta =
          ((_minNano - nanoseconds) / TimeConstants.nanosecondsPerMillisecond)
              .ceil();
      milliseconds -= delta;
  ...
  factory Time.bigIntNanoseconds(BigInt bigNanoseconds) {
    ...
    var milliseconds =
        (bigNanoseconds ~/ TimeConstants.nanosecondsPerMillisecondBigInt)
            .toInt();
    var nanoseconds = bigArithmeticMod(
            bigNanoseconds, TimeConstants.nanosecondsPerMillisecondBigInt)
        .toInt();
  ...
  factory Time(
    ...
    var _milliseconds =
        (_seconds + seconds) * TimeConstants.millisecondsPerSecond +
            milliseconds;

    var _microseconds = (_milliseconds - _milliseconds.toInt()) *
    ...
    if (microseconds.abs() > Platform.maxMicrosecondsToNanoseconds) {
      var millisecondsToAdd =
          microseconds ~/ TimeConstants.microsecondsPerMillisecond;
      _milliseconds += millisecondsToAdd;
      microseconds -=
          millisecondsToAdd * TimeConstants.microsecondsPerMillisecond;
    }

    var _nanoseconds = (_microseconds + microseconds) *
  ...
  factory Time.duration(Duration duration) {
    ...
    var nanosecondsInterval = TimeConstants.nanosecondsPerMicrosecond *
        (duration.inMicroseconds -
            duration.inMilliseconds * TimeConstants.microsecondsPerMillisecond);
  ...
  // todo: hour of Day?
  int get hourOfDay => arithmeticMod(
      (_milliseconds ~/ TimeConstants.millisecondsPerHour),
      TimeConstants.hoursPerDay);
  int get minuteOfHour => arithmeticMod(
      (_milliseconds ~/ TimeConstants.millisecondsPerMinute),
      TimeConstants.minutesPerHour);
  int get secondOfMinute => arithmeticMod(
      (_milliseconds ~/ TimeConstants.millisecondsPerSecond),
      TimeConstants.secondsPerMinute);
  int get millisecondOfSecond =>
      arithmeticMod(_milliseconds, TimeConstants.millisecondsPerSecond);
  ...
  double get totalDays =>
      ...
      _nanosecondsInterval / TimeConstants.nanosecondsPerDay;
  double get totalHours =>
      _milliseconds / TimeConstants.millisecondsPerHour +
      _nanosecondsInterval / TimeConstants.nanosecondsPerHour;
  double get totalMinutes =>
      _milliseconds / TimeConstants.millisecondsPerMinute +
      _nanosecondsInterval / TimeConstants.nanosecondsPerMinute;
  double get totalSeconds =>
      _milliseconds / TimeConstants.millisecondsPerSecond +
      _nanosecondsInterval / TimeConstants.nanosecondsPerSecond;
  double get totalMilliseconds =>
      _milliseconds +
      _nanosecondsInterval / TimeConstants.nanosecondsPerMillisecond;
  double get totalMicroseconds =>
      _milliseconds * TimeConstants.microsecondsPerMillisecond +
      _nanosecondsInterval / TimeConstants.nanosecondsPerMicrosecond;
  double get totalNanoseconds => canNanosecondsBeInteger
      ? inNanoseconds.toDouble()
      : inNanosecondsAsBigInt.toDouble();

  BigInt get inNanosecondsAsBigInt =>
      BigInt.from(_milliseconds) *
          TimeConstants.nanosecondsPerMillisecondBigInt +
      BigInt.from(_nanosecondsInterval);
  ...
  bool get canNanosecondsBeInteger =>
      _milliseconds <
          Platform.intMaxValue ~/ TimeConstants.nanosecondsPerMillisecond &&
      _milliseconds >
          Platform.intMinValue ~/ TimeConstants.nanosecondsPerMillisecond;

  bool get isNegative =>
      _milliseconds < 0 || (_milliseconds == 0 && _nanosecondsInterval < 0);
  ...
  */
  ...
  @override
  get hashCode => _milliseconds.hashCode ^ _nanosecondsInterval;
  ...
  // todo: test
  Time abs(Time other) =>
      Time._untrusted(_milliseconds.abs(), _nanosecondsInterval.abs());

  Time add(Time other) => Time._untrusted(_milliseconds + other._milliseconds,
      _nanosecondsInterval + other._nanosecondsInterval);

  Time subtract(Time other) => Time._untrusted(
      _milliseconds - other._milliseconds,
      _nanosecondsInterval - other._nanosecondsInterval);

  Time operator *(num factor) => Time._untrusted(
      (_milliseconds * factor).floor(),
      (_nanosecondsInterval * factor).floor());
  ...
  Time _plusSmallNanoseconds(int nanoseconds) =>
      Time._untrusted(_milliseconds, _nanosecondsInterval + nanoseconds);

  @override
  bool operator ==(dynamic other) => other is Time && equals(other);

  bool operator >=(Time other) => other == null
      ? true
      : (_milliseconds > other._milliseconds) ||
          (_milliseconds == other._milliseconds &&
              _nanosecondsInterval >= other._nanosecondsInterval);
  ...
  int compareTo(Time other) {
    ...
    return millisecondsComparison != 0
        ...
        : _nanosecondsInterval.compareTo(other._nanosecondsInterval);
  }
  ...
  int get _nanosecondOfDurationDay {
    ...
    return arithmeticMod(_milliseconds, TimeConstants.millisecondsPerDay) *
            TimeConstants.nanosecondsPerMillisecond +
        _nanosecondsInterval;
  ...
  int get _nanosecondOfEpochDay {
    ...
    return epochArithmeticMod(_milliseconds, TimeConstants.millisecondsPerDay) *
            TimeConstants.nanosecondsPerMillisecond +
        _nanosecondsInterval;
  }
}
...
class MillisecondTime extends Time {
  ...
  const MillisecondTime(this._milliseconds, this._nanosecondsInterval)
      : super._();
}
...
class NanosecondTime extends Time {
  ...
  @override
  int get _milliseconds => inMilliseconds;
  @override
  int get _nanosecondsInterval =>
      arithmeticMod(_nanoseconds, TimeConstants.nanosecondsPerMillisecond);

  NanosecondTime(this._nanoseconds) : super._() {
    assert(this._nanoseconds >= Platform.intMinValue &&
        this._nanoseconds <= Platform.intMaxValue);
  }

  @override
  Time operator -() => NanosecondTime(-_nanoseconds);
  @override
  bool operator <(Time other) => _nanoseconds < other.totalNanoseconds;
  @override
  bool operator <=(Time other) => _nanoseconds <= other.totalNanoseconds;
  @override
  bool operator >(Time other) => _nanoseconds > other.totalNanoseconds;
  ...
  @override
  Time _plusSmallNanoseconds(int nanoseconds) =>
      NanosecondTime(nanoseconds + _nanoseconds);

  @override
  Time abs(Time other) => NanosecondTime(_nanoseconds.abs());
  @override
  bool get isNegative => _nanoseconds.isNegative;

  @override
  bool get canNanosecondsBeInteger => true;
  ...
  @override
  bool equals(Time other) => other.canNanosecondsBeInteger
      ? _nanoseconds == other.inNanoseconds
      : false;

  @override
  int get hourOfDay => arithmeticMod(
      (_nanoseconds ~/ TimeConstants.nanosecondsPerHour),
      TimeConstants.hoursPerDay);
  @override
  int get minuteOfHour => arithmeticMod(
      (_nanoseconds ~/ TimeConstants.nanosecondsPerMinute),
      TimeConstants.minutesPerHour);
  @override
  int get secondOfMinute => arithmeticMod(
      (_nanoseconds ~/ TimeConstants.nanosecondsPerSecond),
      TimeConstants.secondsPerMinute);
  @override
  int get millisecondOfSecond => arithmeticMod(
      _nanoseconds ~/ TimeConstants.nanosecondsPerMillisecond,
      TimeConstants.millisecondsPerSecond);
  // todo: is [mod, division] or [division, mod] better?
  @override
  int get microsecondOfSecond =>
      arithmeticMod(_nanoseconds, TimeConstants.nanosecondsPerSecond) ~/
      TimeConstants.microsecondsPerMillisecond;
  @override
  int get nanosecondOfSecond =>
      arithmeticMod(_nanoseconds, TimeConstants.nanosecondsPerSecond);
  ...
  @override
  double get totalDays => _nanoseconds / TimeConstants.nanosecondsPerDay;
  @override
  double get totalHours => _nanoseconds / TimeConstants.nanosecondsPerHour;
  @override
  double get totalMinutes => _nanoseconds / TimeConstants.nanosecondsPerMinute;
  @override
  double get totalSeconds => _nanoseconds / TimeConstants.nanosecondsPerSecond;
  @override
  double get totalMilliseconds =>
      _nanoseconds / TimeConstants.nanosecondsPerMillisecond;
  @override
  double get totalMicroseconds =>
      _nanoseconds / TimeConstants.nanosecondsPerMicrosecond;
  @override
  double get totalNanoseconds => _nanoseconds.toDouble();

  @override
  BigInt get inNanosecondsAsBigInt => BigInt.from(_nanoseconds);

  @override
  int get inDays => _nanoseconds ~/ TimeConstants.nanosecondsPerDay;
  @override
  int get inHours => _nanoseconds ~/ TimeConstants.nanosecondsPerHour;
  @override
  int get inMinutes => _nanoseconds ~/ TimeConstants.nanosecondsPerMinute;
  @override
  int get inSeconds => _nanoseconds ~/ TimeConstants.nanosecondsPerSecond;
  @override
  ...
  int get inMicroseconds =>
      _nanoseconds ~/ TimeConstants.nanosecondsPerMicrosecond;
  @override
  int get inNanoseconds => _nanoseconds;

  @override
  Duration get toDuration => Duration(microseconds: inMicroseconds);
