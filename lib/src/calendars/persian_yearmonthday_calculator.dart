...
@internal
abstract class PersianYearMonthDayCalculator
    extends RegularYearMonthDayCalculator {
  ...
  static const int _averageDaysPer10Years =
      (_daysPerNonLeapYear * 25 + _daysPerLeapYear * 8) * 10 ~/ 33;
  ...
  /*final*/
  List<int> get _startOfYearInDaysCache =>
      __startOfYearInDaysCache ??= _generateStartOfYearInDaysCache();
  ...
  PersianYearMonthDayCalculator._(int daysAtStartOfYear1)
      : super(
            1, maxPersianYear, 12, _averageDaysPer10Years, daysAtStartOfYear1);

  @protected
  @override
  int getDaysFromStartOfYearToStartOfMonth(int year, int month) =>
      _totalDaysByMonth[month];
  ...
  @override
  int getDaysInMonth(int year, int month) => month < 7
      ? 31
      : month < 12
          ? 30
          : isLeapYear(year)
...
class PersianSimple extends PersianYearMonthDayCalculator {
  ...
  static const int _leapYearPatternBits = (1 << 1) |
      (1 << 5) |
      (1 << 9) |
      (1 << 13) |
      (1 << 17) |
...
class PersianAstronomical extends PersianYearMonthDayCalculator {
  ...
  @override
  bool isLeapYear(int year) =>
      (_astronomicalLeapYearBits[year >> 3] & (1 << (year & 7))) != 0;
}
