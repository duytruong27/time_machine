...
class _Constructor {
  ...
  static final List<int> _monthStartDays =
      _gregorianYearMonthDayCalculator_Init()[0];

  // The 1-based days-since-unix-epoch for the start of each year
  static final List<int> _yearStartDays =
      _gregorianYearMonthDayCalculator_Init()[1];
  ...
  static List<List<int>> _gregorianYearMonthDayCalculator_Init() {
    if (_gregorianYearMonthDayCalculator_Initialized != null)
      return _gregorianYearMonthDayCalculator_Initialized;

    var _monthStartDays =
        List<int>((_lastOptimizedYear + 1 - _firstOptimizedYear) * 12 + 1);
    var _yearStartDays =
        List<int>(_lastOptimizedYear + 1 - _firstOptimizedYear);
    ...
    _gregorianYearMonthDayCalculator_Initialized = [
      _monthStartDays,
      _yearStartDays
    ];
    return _gregorianYearMonthDayCalculator_Initialized;
...
class GregorianYearMonthDayCalculator extends GJYearMonthDayCalculator {
  ...
  // The 0-based days-since-unix-epoch for the start of each month
  static final List<int> _monthStartDays = _Constructor
      ._monthStartDays; //new List<int>((_lastOptimizedYear + 1 - _firstOptimizedYear) * 12 + 1);

  // The 1-based days-since-unix-epoch for the start of each year
  static final List<int> _yearStartDays = _Constructor
      ._yearStartDays; // new List<int>(_lastOptimizedYear + 1 - _firstOptimizedYear);
  ...
  static YearMonthDayCalendar
      getGregorianYearMonthDayCalendarFromDaysSinceEpoch(int daysSinceEpoch) {
    if (daysSinceEpoch < _firstOptimizedDay ||
        daysSinceEpoch > _lastOptimizedDay) {
      return ICalendarSystem.getYearMonthDayCalendarFromDaysSinceEpoch(
          CalendarSystem.iso, daysSinceEpoch);
    }
  ...
  GregorianYearMonthDayCalculator()
      : super(minGregorianYear, maxGregorianYear, _averageDaysPer10Years,
            -719162) {
  ...
  static void validateGregorianYearMonthDay(int year, int month, int day) {
    ...
    if (year < minGregorianYear ||
        year > maxGregorianYear ||
        month < 1 ||
        month > 12) {
      Preconditions.checkArgumentRange(
          'year', year, minGregorianYear, maxGregorianYear);
    ...
    int daysInMonth = month == 2 && _isGregorianLeapYear(year)
        ? GJYearMonthDayCalculator.maxDaysPerMonth[month - 1]
        : GJYearMonthDayCalculator.minDaysPerMonth[month - 1];
  ...
  int calculateStartOfYearDays(int year) {
    ...
    if (year < 0) {
      ...
      leapYears = safeRightShift(year + 3, 2) -
          leapYears +
          safeRightShift(leapYears + 3, 2) -
          1;
    } else {
  ...
  static bool _isGregorianLeapYear(int year) =>
      ((year & 3) == 0) &&
      (arithmeticMod(year, 100) != 0 || arithmeticMod(year, 400) == 0);
}
