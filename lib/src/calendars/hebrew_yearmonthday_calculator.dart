...
class HebrewYearMonthDayCalculator extends YearMonthDayCalculator {
  ...
  HebrewYearMonthDayCalculator(this._monthNumbering)
      : super(
            HebrewScripturalCalculator.minYear,
  ...
  int _calendarToCivilMonth(int year, int month) =>
      _monthNumbering == HebrewMonthNumbering.civil
          ? month
          : HebrewMonthConverter.scripturalToCivil(year, month);

  int _calendarToScripturalMonth(int year, int month) =>
      _monthNumbering == HebrewMonthNumbering.scriptural
          ? month
          : HebrewMonthConverter.civilToScriptural(year, month);
  ...
  @override
  bool isLeapYear(int year) => HebrewScripturalCalculator.isLeapYear(year);

  @protected
  @override
  int getDaysFromStartOfYearToStartOfMonth(int year, int month) {
    int scripturalMonth = _calendarToScripturalMonth(year, month);
    return HebrewScripturalCalculator.getDaysFromStartOfYearToStartOfMonth(
        year, scripturalMonth);
  }
  ...
  int calculateStartOfYearDays(int year) {
    ...
    int daysSinceHebrewEpoch = HebrewScripturalCalculator.elapsedDays(year) -
        1; // ElapsedDays returns 1 for year 1.
    return daysSinceHebrewEpoch + _unixEpochDayAtStartOfYear1;
  }

  @override
  YearMonthDay getYearMonthDay(int year, int dayOfYear) {
    YearMonthDay scriptural =
        HebrewScripturalCalculator.getYearMonthDay(year, dayOfYear);
    return _monthNumbering == HebrewMonthNumbering.scriptural
        ? scriptural
        : YearMonthDay(
            year,
            HebrewMonthConverter.scripturalToCivil(year, scriptural.month),
            scriptural.day);
  }

  @override
  int getDaysInYear(int year) => HebrewScripturalCalculator.daysInYear(year);

  @override
  int getMonthsInYear(int year) => isLeapYear(year) ? 13 : 12;
  ...
  YearMonthDay setYear(YearMonthDay yearMonthDay, int year) {
    ...
    int targetScripturalMonth =
        _calendarToScripturalMonth(currentYear, currentMonth);
    ...
    } else if (targetScripturalMonth == 12 &&
        isLeapYear(year) &&
    ...
    if (targetDay == 30 &&
        (targetScripturalMonth == 8 ||
            targetScripturalMonth == 9 ||
            targetScripturalMonth == 12)) {
      if (HebrewScripturalCalculator.daysInMonth(year, targetScripturalMonth) !=
          30) {
        targetDay = 1;
    ...
    int targetCalendarMonth =
        _scripturalToCalendarMonth(year, targetScripturalMonth);
    return YearMonthDay(year, targetCalendarMonth, targetDay);
  }

  @override
  int getDaysInMonth(int year, int month) =>
      HebrewScripturalCalculator.daysInMonth(
          year, _calendarToScripturalMonth(year, month));
  ...
  int monthsBetween(YearMonthDay start, YearMonthDay end) {
    ...
    double startTotalMonths = startCivilMonth +
        (start.year * _monthsPerLeapCycle) / _yearsPerLeapCycle;
    int endCivilMonth = _calendarToCivilMonth(end.year, end.month);
    double endTotalMonths =
        endCivilMonth + (end.year * _monthsPerLeapCycle) / _yearsPerLeapCycle;
  ...
  @override
  int compare(YearMonthDay lhs, YearMonthDay rhs) {
