...
abstract class IInstant {
  ...
  static final Instant beforeMinValue = Instant._trusted(
      Time(days: ITime.minDays)); //, deliberatelyInvalid: true);
  /// Instant which is invalid *except* for comparison purposes; it is later than any valid value.
  /// This must never be exposed.
  static final Instant afterMaxValue = Instant._trusted(
      Time(days: ITime.maxDays)); //, deliberatelyInvalid: true);

  // note: Extensions would be `better than sliced bread` here!!!!
  static LocalInstant plusOffset(Instant instant, Offset offset) =>
      instant._plusOffset(offset);
  static LocalInstant safePlus(Instant instant, Offset offset) =>
      instant._safePlus(offset);
}
...
class Instant implements Comparable<Instant> {
  ...
  static final Instant minValue =
      Instant._trusted(Time(days: IInstant.minDays));

  /// Represents the largest possible [Instant].
  /// This value is equivalent to 9999-12-31T23:59:59.999999999Z
  static final Instant maxValue = Instant._trusted(Time(
      days: IInstant.maxDays,
      nanoseconds: TimeConstants.nanosecondsPerDay - 1));
  ...
  factory Instant.fromEpochSeconds(int seconds) => Instant.epochTime(
      ITime.trusted(seconds * TimeConstants.millisecondsPerSecond));
  factory Instant.fromEpochMilliseconds(int milliseconds) =>
      Instant.epochTime(ITime.trusted(milliseconds));
  ...
  factory Instant.fromEpochNanoseconds(int nanoseconds) =>
      Instant.epochTime(ITime.trusted(0, nanoseconds));
  factory Instant.fromEpochBigIntNanoseconds(BigInt nanoseconds) =>
      Instant.epochTime(Time.bigIntNanoseconds(nanoseconds));

  // Convenience methods, todo: convert to be like LocalDateTime?
  factory Instant.utc(int year, int monthOfYear, int dayOfMonth, int hourOfDay,
      int minuteOfHour,
      [int secondOfMinute = 0]) {
    var days = LocalDate(year, monthOfYear, dayOfMonth).epochDay;
    var nanoOfDay = LocalTime(hourOfDay, minuteOfHour, secondOfMinute)
        .timeSinceMidnight
        .inNanoseconds;
    return Instant._trusted(Time(days: days, nanoseconds: nanoOfDay));
  }

  factory Instant.julianDate(double julianDate) =>
      TimeConstants.julianEpoch + Time(days: julianDate);

  factory Instant.dateTime(DateTime dateTime) {
    if (Platform.isVM)
      return Instant._trusted(
          Time(microseconds: dateTime.microsecondsSinceEpoch));
    return Instant._trusted(
        Time(milliseconds: dateTime.millisecondsSinceEpoch));
  }

  int compareTo(Instant other) =>
      timeSinceEpoch.compareTo(other.timeSinceEpoch);
  @wasInternal
  bool get isValid => this >= minValue && this <= maxValue;

  @override
  int get hashCode => timeSinceEpoch.hashCode;
  @override
  bool operator ==(dynamic other) =>
      other is Instant && timeSinceEpoch == other.timeSinceEpoch;
  ...
  LocalInstant _safePlus(Offset offset) {
    ...
    // todo: much simplify
    var asDuration = Instant._trusted(
        ITime.plusSmallNanoseconds(timeSinceEpoch, offset.inNanoseconds));
  ...
  Time timeUntil(Instant instant) =>
      instant.timeSinceEpoch.subtract(timeSinceEpoch);

  /// The fluent opposite of [timeUntil]
  Time timeSince(Instant instant) =>
      timeSinceEpoch.subtract(instant.timeSinceEpoch);
  ...
  double toJulianDate() =>
      (TimeConstants.julianEpoch.timeUntil(this)).totalDays;
  ...
  DateTime toDateTimeLocal() => DateTime.fromMillisecondsSinceEpoch(
      timeSinceEpoch.totalMilliseconds.toInt());
  ...
  int get epochSeconds => timeSinceEpoch.totalSeconds
      .floor(); // epochDay*TimeConstants.secondsPerDay + timeOfEpochDay.inSeconds;
  int get epochMilliseconds => timeSinceEpoch.totalMilliseconds
      .floor(); // epochDay*TimeConstants.millisecondsPerDay + timeOfEpochDay.inMilliseconds;
  int get epochMicroseconds => timeSinceEpoch.totalMicroseconds.floor();
  int get epochNanoseconds => timeSinceEpoch.inNanoseconds;
  bool get canEpochNanosecondsBeInteger =>
      timeSinceEpoch.canNanosecondsBeInteger;
  ...
  Time get epochDayTime {
    ...
    return ITime.untrusted(
        ms - epochDay * TimeConstants.millisecondsPerDay, ns);
  }
  ...
  ZonedDateTime inUtc() {
    ...
    var offsetDateTime = IOffsetDateTime.fullTrust(
        LocalDateTime.localDateAtTime(LocalDate.fromEpochDay(epochDay),
            LocalTime.sinceMidnight(epochDayTime)),
        Offset.zero);
  ...
  ZonedDateTime inLocalZone([CalendarSystem calendar]) =>
      ZonedDateTime(this, DateTimeZone.local, calendar);

  OffsetDateTime withOffset(Offset offset, [CalendarSystem calendar]) =>
      IOffsetDateTime.fromInstant(this, offset, calendar);
}
