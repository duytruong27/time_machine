...
class FixedLengthDatePeriodField implements IDatePeriodField {
  ...
  LocalDate add(LocalDate localDate, int value) {
    ...
    if (daysToAdd < 300 && daysToAdd > -300) {
      YearMonthDayCalculator calculator =
      ...
      if (1 <= newDayOfMonth &&
          newDayOfMonth <= calculator.getDaysInMonth(year, month)) {
        return ILocalDate.trusted(YearMonthDayCalendar(
            year, month, newDayOfMonth, ICalendarSystem.ordinal(calendar)));
      }
      ...
      if (newDayOfYear < 1) {
        ...
        if (year < calculator.minYear) {
          throw RangeError(
              'Date computation would underflow the minimum year of the calendar');
        }
      } else {
        ...
        if (newDayOfYear > daysInYear) {
          ...
          if (year > calculator.maxYear) {
            throw RangeError(
                'Date computation would overflow the maximum year of the calendar');
          }
        }
      }
      return ILocalDate.trusted(calculator
          .getYearMonthDay(year, newDayOfYear)
          .withCalendarOrdinal(ICalendarSystem.ordinal(calendar)));
    }
